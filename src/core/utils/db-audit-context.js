 ...
 */
exports.applyAuditContext = async (req, { actorId, actorUsername, requestId } = {}) => {
  const actorIdNum = Number(actorId);
  const actorIdSafe = Number.isFinite(actorIdNum) && actorIdNum > 0 ? Math.trunc(actorIdNum) : null;

  if (!actorIdSafe) throw badReq('actorId kosong / tidak valid. Controller harus inject actorId dari token.');

  const actorSafe = String(actorUsername || '').trim() || null;
  const ridSafe =
    String(requestId || '').trim() || `${Date.now()}-${Math.random().toString(16).slice(2)}`;
