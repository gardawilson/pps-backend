...
exports.update = async (req, res) => {
  ...
  try {
    ...
    if (!NoBroker) {
      return res.status(400).json({ success: false, message: 'nobroker wajib diisi' });
    }
...
exports.getPartialInfo = async (req, res) => {
  ...
  try {
    ...
    const data = await brokerService.getPartialInfoByBrokerAndSak(NoBroker, Math.trunc(NoSakNum));
