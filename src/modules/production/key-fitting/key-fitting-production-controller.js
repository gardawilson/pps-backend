...
async function upsertInputs(req, res) {
  ...
  try {
    const result = await keyFittingService.upsertInputsAndPartials(noProduksi, payload);
    ...
    console.error('[keyFitting.upsertInputs]', e);
    return res.status(500).json({
      success: false,
      message: 'Internal Server Error',
      error: {
...
async function deleteInputsAndPartials(req, res) {
  ...
  try {
    const result = await keyFittingService.deleteInputsAndPartials(noProduksi, payload);
    ...
    console.error('[keyFitting.deleteInputsAndPartials]', e);
    return res.status(500).json({
      success: false,
      message: 'Internal Server Error',
      error: {
