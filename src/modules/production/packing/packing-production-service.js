...
async function createPackingProduksi(payload) {
  const must = [];
  ...
  try {
    ...
    // 1) generate NoPacking BD.0000000138
    ...
    // optional anti-race double check (same as spanner)
    const rqCheck = new sql.Request(tx);
    const exist = await rqCheck
      .input('NoPacking', sql.VarChar(50), no1)
      .query(`
        SELECT 1
    ...
    const noPacking = exist.recordset.length
    ...
    const insertSql = `
      ...
      OUTPUT INSERTED.*
      VALUES (
        @NoPacking, @Tanggal, @IdMesin, @IdOperator, @Shift,
        @JamKerja,
        @CreateBy, @CheckBy1, @CheckBy2, @ApproveBy, @HourMeter,
 ...
 */
async function updatePackingProduksi(noPacking, payload) {
  ...
  try {
    ...
    const { docDateOnly: oldDocDateOnly } = await loadDocDateOnlyFromConfig({
      entityKey: 'packingProduksi', // ✅ must exist in your config
      codeValue: noPacking, // ✅ NoPacking
      runner: tx,
      useLock: true,
      throwIfNotFound: true,
    });

    // 1) if user sends tglProduksi -> new date
    ...
    // 4) if tanggal berubah -> sync FurnitureWIP.DateUsage (FULL + PARTIAL)
    if (isChangingDate && updatedHeader) {
      ...
      const sqlUpdateUsage = `
        /* =======================
           PACKING -> FWIP DateUsage Sync
           ======================= */

        -- FULL (via PackingProduksiInputLabelFWIP)
        UPDATE fw
        ...
        -- PARTIAL (via PackingProduksiInputLabelFWIPPartial -> FurnitureWIPPartial -> FurnitureWIP)
        UPDATE fw
        SET fw.DateUsage = @Tanggal
        FROM dbo.FurnitureWIP AS fw
        WHERE fw.DateUsage IS NOT NULL
...
async function deletePackingProduksi(noPacking) {
  if (!noPacking) throw badReq('noPacking wajib');
  ...
  try {
    ...
    req.input('NoPacking', sql.VarChar(50), noPacking);
    ...
    const sqlDelete = `
      ...
      /* =======================
         B) collect FWIP keys (PARTIAL -> FurnitureWIPPartial)
         ======================= */
      INSERT INTO @FWIPKeys (NoFurnitureWIP)
      ...
      /* =======================
         C) delete partial rows used by this packing
         (same behavior as spanner delete)
         ======================= */
      DELETE fwp
      ...
      /* =======================
         D) delete mappings (partial & full) + material inputs
         ======================= */
      DELETE FROM dbo.PackingProduksiInputLabelFWIPPartial
      WHERE NoPacking = @NoPacking;

      DELETE FROM dbo.PackingProduksiInputLabelFWIP
      WHERE NoPacking = @NoPacking;

      DELETE FROM dbo.PackingProduksiInputMaterial
      WHERE NoPacking = @NoPacking;
 ...
 * }
 */
async function upsertInputsAndPartials(noPacking, payload) {
  if (!noPacking) throw badReq('noPacking wajib');
  ...
  try {
    ...
    const { docDateOnly } = await loadDocDateOnlyFromConfig({
      entityKey: 'packingProduksi', // ✅ must match config key
      codeValue: noPacking,
      runner: tx,
      useLock: true,
      throwIfNotFound: true,
    ...
    await assertNotLocked({
      date: docDateOnly,
      runner: tx,
      action: 'upsert Packing inputs/partials',
      useLock: true,
    });

    // 2) create partial + mapping
    const partials = await _insertPartialsWithTx(tx, noPacking, {
      furnitureWipPartialNew: body.furnitureWipPartialNew,
    });

    // 3) attach existing inputs
    const fwipAttach = await _insertFurnitureWipWithTx(tx, noPacking, {
      furnitureWip: body.furnitureWip,
    });

    const matAttach = await _insertCabinetMaterialWithTx(tx, noPacking, {
...
/* =====================
   SQL helpers (Packing tables)
===================== */

async function _insertPartialsWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  ...
  WHERE h.NoPacking = @no;

  IF @tgl IS NULL
  BEGIN
    RAISERROR('Header PackingProduksi_h tidak ditemukan / Tanggal NULL', 16, 1);
    RETURN;
  END;
  ...
  BEGIN
    ...
    INSERT INTO dbo.PackingProduksiInputLabelFWIPPartial (NoPacking, NoFurnitureWIPPartial)
    SELECT @no, c.NoFurnitureWIPPartial
    FROM @createdFWP c
    WHERE NOT EXISTS (
      SELECT 1 FROM dbo.PackingProduksiInputLabelFWIPPartial x WITH (NOLOCK)
      WHERE x.NoPacking=@no AND x.NoFurnitureWIPPartial=c.NoFurnitureWIPPartial
    );
...
async function _insertFurnitureWipWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  ...
  FROM @reqFW r
  WHERE EXISTS (
    SELECT 1 FROM dbo.PackingProduksiInputLabelFWIP x WITH (NOLOCK)
    WHERE x.NoPacking=@no AND x.NoFurnitureWIP=r.NoFurnitureWip
  );
  ...
  INSERT INTO dbo.PackingProduksiInputLabelFWIP (NoPacking, NoFurnitureWIP)
...
async function _insertCabinetMaterialWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  req.input('jsInputs', sql.NVarChar(sql.MAX), JSON.stringify(lists));
  ...
  FROM dbo.PackingProduksiInputMaterial tgt
  JOIN @MatSrc src ON src.IdCabinetMaterial=tgt.IdCabinetMaterial
  WHERE tgt.NoPacking=@no
    AND src.Jumlah > 0
    AND EXISTS (
  ...
  INSERT INTO dbo.PackingProduksiInputMaterial (NoPacking, IdCabinetMaterial, Jumlah)
  ...
  WHERE src.Jumlah > 0
    ...
    AND NOT EXISTS (
      SELECT 1 FROM dbo.PackingProduksiInputMaterial x WITH (NOLOCK)
      WHERE x.NoPacking=@no AND x.IdCabinetMaterial=src.IdCabinetMaterial
    );
...
async function deleteInputsAndPartials(noPacking, payload) {
  if (!noPacking) throw badReq('noPacking wajib');
  ...
  try {
    ...
    await assertNotLocked({
      date: docDateOnly,
      runner: tx,
      action: 'delete Packing inputs/partials',
      useLock: true,
    });

    // 2) delete each section
    const partialsRes = await _deletePartialsWithTx(tx, noPacking, {
      furnitureWipPartial: body.furnitureWipPartial,
    });

    const fwRes = await _deleteFurnitureWipWithTx(tx, noPacking, {
      furnitureWip: body.furnitureWip,
    });

    const matRes = await _deleteCabinetMaterialWithTx(tx, noPacking, {
      cabinetMaterial: body.cabinetMaterial,
    });
    ...
    const summary = {
      ...
      furnitureWipPartial:
        partialsRes?.summary?.furnitureWipPartial ?? { deleted: 0, notFound: 0 },
    };
...
/* =====================================================
   DELETE PARTIALS (PackingProduksiInputLabelFWIPPartial + FurnitureWIPPartial)
===================================================== */
async function _deletePartialsWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  req.input('jsPartials', sql.NVarChar(sql.MAX), JSON.stringify(lists));
  ...
  IF @tgl IS NULL
  BEGIN
    RAISERROR('Header PackingProduksi_h tidak ditemukan', 16, 1);
    RETURN;
  END;
  ...
  -- count deletable mappings (exist in map)
  SELECT @fwpDeleted = COUNT(*)
  FROM dbo.PackingProduksiInputLabelFWIPPartial map
  ...
  -- collect affected parent FWIP (for IsPartial/DateUsage recompute)
  ...
  INNER JOIN dbo.PackingProduksiInputLabelFWIPPartial map
  ...
  -- recompute parent FWIP flags
  IF @fwpDeleted > 0
  BEGIN
    -- still has partials -> IsPartial=1, DateUsage NULL
    UPDATE f
    SET
      f.DateUsage = NULL,
      f.IsPartial = 1
    ...
    -- no more partials -> IsPartial=0, DateUsage NULL
    UPDATE f
    SET
      f.DateUsage = NULL,
      f.IsPartial = 0
...
/* =====================================================
   DELETE FULL FWIP (PackingProduksiInputLabelFWIP)
===================================================== */
async function _deleteFurnitureWipWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  req.input('jsInputs', sql.NVarChar(sql.MAX), JSON.stringify(lists));
  ...
  BEGIN
    ...
    FROM dbo.FurnitureWIP f
    INNER JOIN dbo.PackingProduksiInputLabelFWIP map ON f.NoFurnitureWIP = map.NoFurnitureWIP
    INNER JOIN OPENJSON(@jsInputs, '$.furnitureWip')
      WITH (noFurnitureWip varchar(50) '$.noFurnitureWip') j
      ON map.NoFurnitureWIP = j.noFurnitureWip
    WHERE map.NoPacking = @no;
...
/* =====================================================
   DELETE MATERIAL (PackingProduksiInputMaterial)
===================================================== */
async function _deleteCabinetMaterialWithTx(tx, noPacking, lists) {
  const req = new sql.Request(tx);
  req.input('no', sql.VarChar(50), noPacking);
  req.input('jsInputs', sql.NVarChar(sql.MAX), JSON.stringify(lists));
  ...
  FROM dbo.PackingProduksiInputMaterial map
...
module.exports = { getAllProduksi, getProduksiByDate, createPackingProduksi, updatePackingProduksi, deletePackingProduksi, fetchInputs, upsertInputsAndPartials, deleteInputsAndPartials };
