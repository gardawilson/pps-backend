...
async function upsertInputsAndPartials(noProduksi, payload) {
  ...
  try {
    ...
    const { docDateOnly } = await loadDocDateOnlyFromConfig({
      entityKey: 'spanner', // ✅ must match your config
      codeValue: noProduksi,
      runner: tx,
      useLock: true,
      throwIfNotFound: true,
    ...
    await assertNotLocked({
      date: docDateOnly,
      runner: tx,
      action: 'upsert Spanner inputs/partials',
      useLock: true,
    });
...
async function _insertPartialsWithTx(tx, noProduksi, lists) {
  ...
  IF @tgl IS NULL
  BEGIN
    RAISERROR('Header Spanner_h tidak ditemukan / Tanggal NULL', 16, 1);
    RETURN;
  END;
  ...
  BEGIN
    ...
    INSERT INTO dbo.SpannerInputLabelFWIPPartial (NoProduksi, NoFurnitureWIPPartial)
    SELECT @no, c.NoFurnitureWIPPartial
    FROM @createdFWP c
    WHERE NOT EXISTS (
      SELECT 1 FROM dbo.SpannerInputLabelFWIPPartial x WITH (NOLOCK)
...
async function _insertFurnitureWipWithTx(tx, noProduksi, lists) {
  ...
  FROM @reqFW r
  WHERE EXISTS (
    SELECT 1 FROM dbo.SpannerInputLabelFWIP x WITH (NOLOCK)
  ...
  INSERT INTO dbo.SpannerInputLabelFWIP (NoProduksi, NoFurnitureWIP)
...
async function _insertCabinetMaterialWithTx(tx, noProduksi, lists) {
  ...
  FROM dbo.SpannerInputMaterial tgt
  ...
  INSERT INTO dbo.SpannerInputMaterial (NoProduksi, IdCabinetMaterial, Jumlah)
  ...
  WHERE src.Jumlah > 0
    ...
    AND NOT EXISTS (
      SELECT 1 FROM dbo.SpannerInputMaterial x WITH (NOLOCK)
...
async function deleteInputsAndPartials(noProduksi, payload) {
  ...
  try {
    ...
    await assertNotLocked({
      date: docDateOnly,
      runner: tx,
      action: 'delete Spanner inputs/partials',
      useLock: true,
    });

    // 2) delete each section
    const partialsRes = await _deletePartialsWithTx(tx, noProduksi, {
      furnitureWipPartial: body.furnitureWipPartial,
    });

    const fwRes = await _deleteFurnitureWipWithTx(tx, noProduksi, {
      furnitureWip: body.furnitureWip,
    });

    const matRes = await _deleteCabinetMaterialWithTx(tx, noProduksi, {
      cabinetMaterial: body.cabinetMaterial,
    });
    ...
    const summary = {
      ...
      furnitureWipPartial: partialsRes?.summary?.furnitureWipPartial ?? { deleted: 0, notFound: 0 },
    };
...
/* =====================================================
   DELETE PARTIALS (SpannerInputLabelFWIPPartial + FurnitureWIPPartial)
===================================================== */
async function _deletePartialsWithTx(tx, noProduksi, lists) {
  ...
  IF @tgl IS NULL
  BEGIN
    RAISERROR('Header Spanner_h tidak ditemukan', 16, 1);
    RETURN;
  END;
  ...
  FROM dbo.SpannerInputLabelFWIPPartial map
  ...
  INNER JOIN dbo.SpannerInputLabelFWIPPartial map
...
/* =====================================================
   DELETE FULL FWIP (SpannerInputLabelFWIP)
===================================================== */
async function _deleteFurnitureWipWithTx(tx, noProduksi, lists) {
  ...
  FROM dbo.SpannerInputLabelFWIP map
  ...
  BEGIN
    ...
    FROM dbo.FurnitureWIP f
    INNER JOIN dbo.SpannerInputLabelFWIP map ON f.NoFurnitureWIP = map.NoFurnitureWIP
    INNER JOIN OPENJSON(@jsInputs, '$.furnitureWip')
      WITH (noFurnitureWip varchar(50) '$.noFurnitureWip') j
      ON map.NoFurnitureWIP = j.noFurnitureWip
    WHERE map.NoProduksi = @no;
...
/* =====================================================
   DELETE MATERIAL (SpannerInputMaterial)
===================================================== */
async function _deleteCabinetMaterialWithTx(tx, noProduksi, lists) {
  ...
  FROM dbo.SpannerInputMaterial map
