...
async function noStockOpnameHandler(req, res) {
  console.log(
    `[${new Date().toISOString()}] 🔵 GET /no-stock-opname endpoint hit by user: ${req.user?.username || "unknown"}`,
  );

  try {
    const { page = 1, limit = 10, isAscend, search = "" } = req.query;
    const pageNum = parseInt(page, 10);
    const limitNum = parseInt(limit, 10);
    const isAscendFilter =
      isAscend !== undefined ? parseInt(isAscend, 10) : null;
...
async function validateStockOpnameLabelHandler(req, res) {
  ...
  try {
    const result = await validateStockOpnameLabel({
      noso,
      label,
      username,
      blok,
    ...
    res
      .status(500)
      .json({ message: "Gagal memvalidasi label", error: err.message });
  }
}
...
async function insertStockOpnameLabelHandler(req, res) {
  ...
  try {
    ...
    if (!Number.isInteger(idlokasiNum)) {
      return res
        .status(400)
        .json({ message: "idlokasi harus bertipe integer" });
    }
...
async function stockOpnameFamiliesHandler(req, res) {
  ...
  console.log(
    `[${new Date().toISOString()}] 🔵 GET /no-stock-opname/${noso}/families`,
  );
  ...
  try {
    ...
    if (!families) {
      return res
        .status(404)
        .json({ message: "Family Stock Opname tidak ditemukan" });
    }
    res.json(families);
  } catch (err) {
...
async function stockOpnameAscendDataHandler(req, res) {
  ...
  console.log(
    `[${new Date().toISOString()}] 🔵 GET /no-stock-opname/${noso}/families/${familyid}/ascend?keyword=${keyword}`,
  );

  try {
    const data = await getStockOpnameAscendData({
      noSO: noso,
      familyID: familyid,
      keyword,
    });
...
async function saveStockOpnameAscendHasilHandler(req, res) {
  ...
  if (!Array.isArray(dataList) || dataList.length === 0) {
    return res
      .status(400)
      .json({ message: "dataList harus berupa array dan tidak boleh kosong" });
  }
...
async function deleteStockOpnameHasilAscendHandler(req, res) {
  try {
    ...
    const { deletedCount } = await deleteStockOpnameHasilAscend(
      noso,
      parsedItemId,
    );
    ...
    if (!deletedCount) {
      return res
        .status(404)
        .json({ message: "Data tidak ditemukan / sudah terhapus" });
    }
